...
from transform_config import transform_none, transform_light, transform_light_flip, transform_val

# ---- pick augmentation config via command line ----
parser = argparse.ArgumentParser()
parser.add_argument("--aug", type=str, default="none", choices=["none", "light", "light_flip"])
args = parser.parse_args()

aug_map = {
    "none": transform_none,
    "light": transform_light,
    "light_flip": transform_light_flip,
}
train_transform = aug_map[args.aug]
...
for epoch in range(EPOCHS):
    ...
    if median_haversine < best_val_haversine:
        ...
        torch.save(model.state_dict(), r"checkpoints/data augmentation/best_model_sphere_{args.aug}.pt")
        print(f"  -> saved new best sphere model data aug (median={median_haversine:.1f} km)")

print(f"Training complete [{args.aug}]. Best val median haversine:", best_val_haversine)
...
plt.tight_layout()
plt.savefig(f"checkpoints/data augmentation/training_curves_sphere_{args.aug}.png", dpi=150)
print("Saved training curves to checkpoints/data augmentation/training_curves_sphere.png")